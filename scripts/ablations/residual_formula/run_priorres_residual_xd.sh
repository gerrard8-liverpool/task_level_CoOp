...
VARIANT=${1:?Usage: bash run_priorres_residual_xd.sh <safe|legacy> <source_dataset> <seed> <target1> [target2 ...]}
SOURCE=${2:?Usage: bash run_priorres_residual_xd.sh <safe|legacy> <source_dataset> <seed> <target1> [target2 ...]}
SEED=${3:?Usage: bash run_priorres_residual_xd.sh <safe|legacy> <source_dataset> <seed> <target1> [target2 ...]}
shift 3
TARGETS=("$@")
...
COMMON="source_${SOURCE}/shots_${SHOTS}/CoOpPriorRes/${TRAIN_CFG_TAG}/nctx${NCTX}_csc${CSC}_ctp${CTX_POS}_${VARIANT_TAG}/seed${SEED}"
...
echo "============================================================"
echo "[XD PRIORRES RESIDUAL FORMULA ABLATION: TRAIN]"
...
for TARGET in "${TARGETS[@]}"; do
  ...
  echo "============================================================"
  echo "[XD PRIORRES RESIDUAL FORMULA ABLATION: TEST]"
  echo "variant   = ${VARIANT_TAG}"
  echo "source    = ${SOURCE}"
  echo "target    = ${TARGET}"
  echo "seed      = ${SEED}"
