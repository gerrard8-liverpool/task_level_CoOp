...
SOURCE=${1:?Usage: GPU=0 bash 06b_test_cocoop_priorres_xd_only.sh <source> <seed> <target1> [target2 ...]}
SEED=${2:?Usage: GPU=0 bash 06b_test_cocoop_priorres_xd_only.sh <source> <seed> <target1> [target2 ...]}
shift 2
TARGETS=("$@")
...
if [ ! -f "$TRAIN_DIR/prompt_learner/model.pth.tar-${LOAD_EPOCH}" ]; then
  echo "[ERROR] Missing PriorRes prompt checkpoint: $TRAIN_DIR/prompt_learner/model.pth.tar-${LOAD_EPOCH}"
  exit 1
fi

if [ ! -f "$TRAIN_DIR/prior_adapter/model.pth.tar-${LOAD_EPOCH}" ]; then
  echo "[ERROR] Missing PriorRes adapter checkpoint: $TRAIN_DIR/prior_adapter/model.pth.tar-${LOAD_EPOCH}"
  exit 1
fi
...
for TARGET in "${TARGETS[@]}"; do
  ...
  echo "============================================================"
  echo "[CoCoOpPriorRes XD TEST ONLY]"
  echo "source  = $SOURCE"
  echo "target  = $TARGET"
  echo "seed    = $SEED"
