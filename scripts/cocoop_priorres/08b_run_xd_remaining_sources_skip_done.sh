...
for SOURCE in "${SOURCES[@]}"; do
  for SEED in "${SEEDS[@]}"; do
    echo "============================================================"
    echo "[TRAIN] source=${SOURCE} seed=${SEED} gpu=${GPU}"
    echo "============================================================"

    OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 \
    GPU="$GPU" bash "$ROOT/scripts/cocoop_priorres/05a_train_cocoop_xd_only.sh" "$SOURCE" "$SEED"

    OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 \
    GPU="$GPU" bash "$ROOT/scripts/cocoop_priorres/06a_train_cocoop_priorres_xd_only.sh" "$SOURCE" "$SEED"
    ...
    for TARGET in "${ALL[@]}"; do
      ...
      echo "============================================================"
      echo "[TEST] source=${SOURCE} target=${TARGET} seed=${SEED} gpu=${GPU}"
      echo "============================================================"
