...
class PriorResidualOutput:
    ...
    def as_dict(self) -> Dict[str, torch.Tensor]:
        return {
            ...
            "regime_k": torch.tensor(float(self.regime_k), device=self.a.device),
        }


class PriorResidualAdapter(nn.Module):
    """Task-conditioned prior + residual joint adapter for CoOp.
    ...
    def __init__(
        ...
        self.regime_k = int(regime_k) if regime_k is not None and regime_k > 0 else self.kmax
        self.logit_eps = 1e-6
        ...
        loader = TaskFeatureLoader(
            ...
            regime_k=regime_k if regime_k is not None and regime_k > 0 else None,
            append_regime_feature=append_regime_feature,
        )
        batch = loader.load()
    ...
    def forward(self, epoch: int) -> Dict[str, torch.Tensor]:
        ...
        b = torch.sigmoid(self._safe_logit(b0) + self.delta_b)
    ...
    def compute_lambda_t(self, epoch: int) -> float:
        if epoch is None:
    ...
    def extra_stats(self, epoch: int) -> Dict[str, float]:
        with torch.no_grad():
