...
class CustomCLIPPriorRes(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        regime_k = cfg.DATASET.NUM_SHOTS if getattr(cfg.DATASET, "NUM_SHOTS", -1) > 0 else None
        ...
        self.prior_adapter = PriorResidualAdapter(
            ...
            regime_k=regime_k,
            append_regime_feature=cfg.TRAINER.COOP.APPEND_REGIME_FEATURE,
            device=torch.device("cpu"),
        )

    def forward(self, image: torch.Tensor, epoch: int):
...
class CoOpPriorRes(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if self.epoch < cfg.TRAINER.COOP.WARMUP_EPOCHS:
            loss, output, aux = self._prompt_step(image, label, epoch=self.epoch, warmup_only=True)
            summary = self._build_summary(loss, output, aux, prefix="")
            summary["stage"] = 0.0
        else:
            if cfg.TRAINER.COOP.ALTERNATE_OPT:
                loss_prompt, output_prompt, aux_prompt = self._prompt_step(image, label, epoch=self.epoch, warmup_only=False)
                loss_meta, output_meta, aux_meta = self._meta_step(image, label, epoch=self.epoch)
                ...
                loss, output, aux = self._joint_step(image, label, epoch=self.epoch)
                summary = self._build_summary(loss, output, aux, prefix="")
                summary["stage"] = 1.0
    ...
    def _joint_step(self, image, label, epoch: int):
    ...
    def _prompt_step(self, image, label, epoch: int, warmup_only: bool = False):
        self._set_trainable(prompt_on=True, meta_on=False)
        output, aux = self._model_forward(image, epoch)
        loss = self._compose_total_loss(output, label, aux, meta_regularize=not warmup_only)
        self._zero_all_optimizers()
        self._backward_and_step(loss, self.optim_prompt, zero_all=False)
        return loss.detach(), output.detach(), self._detach_aux(aux)

    def _meta_step(self, image, label, epoch: int):
        self._set_trainable(prompt_on=False, meta_on=True)
        output, aux = self._model_forward(image, epoch)
        loss = self._compose_total_loss(output, label, aux, meta_regularize=True)
        self._zero_all_optimizers()
    ...
    def _compose_total_loss(
        ...
        use_b = bool(cfg.TRAINER.COOP.USE_B)
        weight_scale = float(cfg.TRAINER.COOP.B_LOSS_WEIGHT)
        ...
        if use_b:
            b_loss, b_aux = weighted_fewshot_loss(
                ...
                labels=label,
                b=aux["b"],
                regime_k=int(aux["regime_k"].item()),
    ...
    def _build_summary(self, loss, output, aux, prefix=""):
        summary = {
            f"{prefix}loss": float(loss.item()),
            f"{prefix}acc": float(compute_accuracy(output, self._current_label)[0].item())
            if hasattr(self, "_current_label") else float("nan"),
            f"{prefix}loss_cls": float(aux["loss_cls"].item()),
            f"{prefix}loss_b": float(aux["loss_b"].item()),
            f"{prefix}reg_a": float(aux["reg_a"].item()),
            f"{prefix}reg_b": float(aux["reg_b"].item()),
            f"{prefix}lambda_t": float(aux["lambda_t"].item()),
            f"{prefix}meff": float(aux["meff"].item()),
            f"{prefix}keff": float(aux["keff"].item()),
            f"{prefix}a0_mean": float(aux["a0"].mean().item()),
            f"{prefix}a_mean": float(aux["a"].mean().item()),
            f"{prefix}b0_mean": float(aux["b0"].mean().item()),
            f"{prefix}b_mean": float(aux["b"].mean().item()),
            f"{prefix}delta_a_norm": float(aux["delta_a"].norm().item()),
            f"{prefix}delta_b_norm": float(aux["delta_b"].norm().item()),
        }
        if "slot_weight_mean" in aux:
            summary[f"{prefix}slot_weight_mean"] = float(aux["slot_weight_mean"].item())
            summary[f"{prefix}slot_weight_max"] = float(aux["slot_weight_max"].item())
        return summary
